        ...
        <div class="row justify-content-center">
            <div class="col-md-8 col-lg-6">
                <form method="POST" action="agendamento/agendar_corte.php" id="agendamentoForm">
                    <?php
                    echo htmlspecialchars($mongo->selectOne("barbearias", ["_id" => $_GET["id"]])->nome);
                    ?>
                    </h1>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="id_barbeiro" id="id_barbeiro" required>
                            ...
                            <?php
                            $barbeiros = $mongo->select("barbeiros", ["barbearia_id" => $_GET["id"]]);

                            foreach ($barbeiros as $barbeiro) {
                                echo "<option value='" . htmlspecialchars($barbeiro->_id) . "' data-foto='" . htmlspecialchars($barbeiro->foto) . "'>" . htmlspecialchars($barbeiro->nome) . "</option>";
                            }
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="horarios" id="horarios" required>
                            ...
                            <?php
                            // Gerar horários fixos de 09:00 às 19:00
                            $horarios = [];
                            for ($i = 9; $i <= 19; $i++) {
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="tipo_corte" id="tipo_corte" required>
                            ...
                            foreach ($tipos_corte as $tipo_corte) {
                                echo "<option value='" . htmlspecialchars($tipo_corte->_id) . "'>" . htmlspecialchars($tipo_corte->nome) . "</option>";
                            }
                            ?>
                        </select>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function atualizarHorarios() {
                ...
                if (idBarbeiro && dataAgendamento) {
                    ...
                    const dataFormatada = dataAgendamento.split('/').reverse().join('-'); // transforma 29/11/2024 em 2024-11-29

                    const url = `agendamento/buscar_horarios.php?id_barbeiro=${idBarbeiro}&data=${dataFormatada}`;
                    console.log('URL de requisição:', url); // Verifique a URL no console
                    ...
                    fetch(url)
                        ...
                        .then(horarios => {
                            console.log('Horários recebidos:', horarios); // Exibe os horários no console
                        ...
                        .catch(error => {
                            console.error('Erro ao buscar horários:', error); // Exibe erro no console
                        });
                }
            }
